...
* **패치**: 배터리 15.5일·최대 착용 14일(넘으면 새 번호 패치로 교체, 시뮬 속도 배율 적용), 리드오프, 교체(착용 만료/배터리/병실 이동/전원/MRI), 퇴원 시 전원 OFF·반납, 교체 시 새 patch_id + NEW_PATCH 플래그. **패치 레지스트리**(`/emr/patches`)가 발급된 모든 패치를 보관합니다: 시리얼·patch_id 단조 증가(재사용 없음), 발급/종료 시각과 사유, 사용 중/사용 완료 상태, 종료 시 배터리.
...
* **모니터링 처방**: 부착 때 위중도(진단·고위험 리듬·75세 이상·동반질환·삽입형 기기·MCOT)로 처방 일수를 정한다 — 단기 3~5일, 중기 7일, 장기 14일(위중하면 일부 7·14일 연장 → 14일째 패치 교체). 처방이 끝나면 모니터링 종료·패치 반납, 빈자리는 새 입원. 시작 때 채우는 환자는 경과를 흩어 한꺼번에 끝나지 않는다. 환자 목록 '처방' 열(처방 일수·D+n·남은 시간·착용일), 환자 상세, `/emr/admissions[].monitoring`. 시나리오 프리셋 **패치 교체 · 처방 종료**, 트리거 `patch_wear_expire`·`patch_low_battery`·`rx_expire` {count, within_s}.
