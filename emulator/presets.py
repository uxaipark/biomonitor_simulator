...
PRESETS = [
    ...
    {"id": "patch_cycle", "name": "패치 교체 · 처방 종료", "purpose": "패치 번호 변경·반납·새 부착 처리",
     "desc": "패치 수명 규칙(최대 착용 14일, 배터리 15.5일, 위중도별 처방 3~14일)을 켜고, 적용 즉시 교체·종료를 몰아서 일으킵니다. 30개는 10분 안에 착용 14일 만료로 새 번호 패치로 바뀌고, 5개는 배터리 교체 기준에 닿아 바로 교체, 20명은 10분 안에 처방이 끝나 반납되거나(장기 일부는 연장) 빈자리에 새 환자가 들어옵니다. 교체 때마다 NEW_PATCH 플래그와 META 가 새 patch_id 를 알립니다.",
     "patch": {"general": {"bed_capacity": 2000, "active_patients": 1000},
               "scenario": {"patch": {"battery_days": 15.5, "max_wear_days": 14, "rx_enabled": True, "battery_drain_enabled": True, "replace_enabled": True, "replace_below_pct": 5}}},
     "actions": [{"what": "patch_wear_expire", "params": {"count": 30, "within_s": 600}}, {"what": "patch_low_battery", "params": {"count": 5}},
                 {"what": "rx_expire", "params": {"count": 20, "within_s": 600}}, {"what": "replace_patch"}]},
...
POINTS.update({'realsig': (['병상 20 · 환자 20명 · 원외 0명, 입퇴원 없음', '슬롯별 ATF/CSV 파일을 표본율 맞춰 반복 재생 (HR 은 파일에서 검출)', '빈 슬롯: 자동 생성 켜면 부정맥 순환 (60초마다)', '장애 · 아티팩트 · 일과 · 에피소드 꺼짐'], ''),
               ...
               'patch_cycle': (['최대 착용 14일 · 배터리 15.5일 · 처방 3~14일 (위중도)', '교체 = 새 serial·patch_id (재사용 없음), NEW_PATCH 플래그 + META', '처방 종료 → 반납·빈자리 새 입원, 장기 일부 연장'],
                               '적용 즉시: 착용 만료 30건(10분) · 배터리 교체 5건 · 처방 종료 20건(10분) · 수동 교체 1건'),
