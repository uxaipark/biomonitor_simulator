...
async function showPatientDetail(id) {                          // detail card for one patient (table row click, plan marker, moving list)
  ...
  X.innerHTML = `<div class="xhead">입원 기록 · 위치 · 이력</div><div class="patient"><img src="/api/v1/emr/patients/${p.id}/avatar.svg"><div class="info"><b>${p.name}</b> <span class="sub">${p.sex === 'M' ? '남' : '여'} ${p.age}세 (${p.birth_date}) · ${p.nationality_label}</span><div>${a && a.patient_no ? `환자번호 #${a.patient_no} · ` : ''}${p.mrn} · 혈액형 ${p.blood_type} · ${p.height_cm}cm ${p.weight_kg}kg BMI ${p.bmi}</div><div>알레르기: ${p.allergies} · 동반질환: ${p.comorbidities.join(', ') || '없음'}</div><div>거주지: ${p.address ? p.address.label : '-'} · ${p.phone || ''}</div></div></div>
    ...
    <tr><td>상태</td><td>${p.status}</td></tr>${a ? `<tr><td>입원</td><td>${a.time} · ${a.ward_name} ${a.bed || ''} · 담당의 ${a.doctor} · 간호사 ${a.nurse} · 패치 ${a.patch}</td></tr>${a.monitoring ? `<tr><td>모니터링 처방</td><td><b>${a.monitoring.tier_ko} ${a.monitoring.days}일</b>${a.monitoring.extended_days ? ` (처방 ${a.monitoring.prescribed_days}일 + 연장 ${a.monitoring.extended_days}일)` : ''} · 위중도 ${a.monitoring.acuity} · ${a.monitoring.start.replace('T', ' ').slice(5, 16)} ~ ${a.monitoring.end.replace('T', ' ').slice(5, 16)}<div class="sub">${esc(a.monitoring.reason)}</div></td></tr>` : ''}<tr><td>검사</td><td>${(a.exams || []).map(e => `${e.time} ${e.type} (${e.room}, ${e.duration_min}분${e.patch_policy === 'remove' ? ', 패치 분리' : ''})${e.done ? ' ✓' : ''}`).join('<br>') || '-'}</td></tr>` : ''}
...
(() => {
  ...
  grid.addEventListener('pointermove', e => {
    ...
    const m = 48, hh = window.innerHeight;
    if (e.clientY < m) window.scrollBy(0, -12); else if (e.clientY > hh - m) window.scrollBy(0, 12);
