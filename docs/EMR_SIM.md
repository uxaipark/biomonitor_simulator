...
## 0. 연동 병원 선택 (에뮬레이터 / 병원 A / 병원 B …)

GUI **EMR 연동 › 연동 병원** 또는 `POST /api/v1/emrsim/link {"site": "emulator" | site_id}` 로 고른다. 선택은 데이터 폴더 `emr_link.json` 에 남아 재시작 후에도 유지된다.

**에뮬레이터 (기본)**
지금 방식 그대로다. 에뮬레이터 자체 EMR(`/api/v1/emr/*`)만 쓰고, 20곳은 각자 가상 환자로 따로 돈다.

**병원 X**
X 의 EMR 이 **에뮬레이터의 실제 모니터링 환자**를 X 의 형식으로 내준다. 나머지 19곳은 독립 가상 병원으로 남는다.

| 항목 | 연동 시 X 가 내주는 것 |
|---|---|
| 환자 | 에뮬레이터 재원 환자 전원(원외 MCOT 포함). 성별·생년월일·진단(ICD-10 → X 의 코드 체계)·동반질환·알레르기는 에뮬레이터 값 그대로다. 이름·식별번호·주소·전화는 X 나라 관례로 만든다. **한국 기관이면 에뮬레이터 이름·주소·전화를 그대로 쓴다.** 같은 환자는 늘 같은 번호를 받는다(에뮬레이터 프로필 id 로 결정). |
| 병동·병실·병상 | 에뮬레이터 병원 구조와 같다. 병동 코드는 `W103A`, 병실은 `103A01`, 병상은 `A`. FHIR Location 에는 도면 병상 id(`urn:biosim:bed` = `103A01-A`)가 붙는다. 병동 밖 병상(응급실 등)은 실 이름별 가상 병동으로, 원외 환자는 `RPM` 원격 모니터링 병동으로 들어간다. |
| 내원번호 | 에뮬레이터 환자번호(`patient_no`, 프레임의 patient_id)를 X 형식으로 바꾼 값이다. 예: Epic CSN, MEDITECH `V00…`, 한국 `YYYYMMDD+6자리`. |
| ADT | 3초마다 에뮬레이터를 따라간다. 입원은 A01, 병상 이동은 A02, 퇴원은 A03 이다. 연동한 순간의 재원 환자는 이벤트 없이 반영되므로 census 계열 API 로 처음 동기화한다. |
| 간호 바이탈 | 입원 15분 뒤부터 4시간 간격으로 쌓인다. 재시작 직후에는 모든 환자가 막 입원한 상태라 한동안 비어 있다. |

**에뮬레이터 쪽 조인 키**
라우터는 프레임의 patient_id 로 연동 EMR 에서 같은 환자를 찾으면 된다.
- `GET /api/v1/emr/admissions` 의 각 행에 `emr: {site, name, mrn, fhir_patient_id, visit, fhir_encounter_id}` 가 붙는다.
- `GET /api/v1/emr/patients/{id}` 에 `emr_link: {identifiers, visit, location, received}` 가 붙는다. `received` 는 이 EMR 이 받은 바이탈 수와 최근값이다.
- GUI 환자 상세에는 '연동 EMR' 줄로 보인다.
